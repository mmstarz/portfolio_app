...
// form data
import { expForm, clearExpForm } from "../profile-forms/addExp";
...
const DashAddExp = ({ addExperience, history }) => {
  const classes = useStyles();
  const [formStatus, setFormStatus] = useState(false);
  const [formSchema, setFormSchema] = useState({ ...expForm });

  const cleanFormState = () => {
    clearExpForm();
    setFormStatus(false);
    setFormSchema({ ...expForm });
  };
  ...
  const handleSubmit = async (e) => {
    ...
    addExperience(dataToSend, history);
...
export default connect(null, mapDispatchToProps)(withRouter(DashAddExp));
